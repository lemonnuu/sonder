...
// 获取 public path，兼容不同构建环境
const getPublicPath = (): string => {
  // 优先使用 Vite/Rsbuild 的环境变量
...
export class LocalProvider implements Provider {
  ...
  async #getPages(lang: string, version: string): Promise<PageIndexInfo[]> {
    ...
    const searchIndexURL = `${removeTrailingSlash(getPublicPath())}/static/${SEARCH_INDEX_NAME}${searchIndexVersion}${searchIndexLang}.${searchIndexHash[searchIndexGroupID]}.json`;
